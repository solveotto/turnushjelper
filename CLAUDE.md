...
Data files live in `turnusdata/{year_id}/` — **outside `app/`, never served directly**. Always reach them through `AppConfig.turnusfiler_dir`, never by building a path from `AppConfig.static_dir`:
- `turnus_schedule_{ID}.json` — full schedule
- `turnus_stats_{ID}.json` — stats/dataframe
- `double_shifts_{id}.json` — double shift data

Active turnus set is stored in `TurnusSet` table (`is_active=1`). `DataframeManager` in `app/utils/df_utils.py` loads and caches the JSON into a pandas DataFrame. `TurnusSet` rows store **absolute** paths, so relocating the data store strands them — `DataframeManager` falls back to the conventional location and logs a warning; `scripts/repoint_turnus_paths.py` makes the DB truthful again.

**Nothing under `app/static/` is authenticated.** The data store moved out of it on 2026-07-29 (Phase 3 item 8): while it lived there, `api.get_shift_image` and `downloads.download_pdf` were `@login_required` but the identical bytes were fetchable at `/static/turnusfiler/…` with no session — protection that didn't protect. `app/static/` now holds only css/js/img. **PII files** (`medlemsliste.xlsx`, `ansinitet.pdf`, `innplassering_{YEAR}.pdf`) live in `instance/protected/` via `app/utils/protected_paths.py` (`AppConfig.protected_dir`). Both rules are enforced by `tests/test_protected_files.py` — note the PII half is a filename denylist and fails open on unanticipated names; the structural guard (no data store under static) is the one that can't.
...
- **Regeneration is slow and needs a raised gunicorn timeout.** `generate_all_images` takes ~15 s locally and ~35 s on the servers for a 423-shift set (~80 ms per render). The admin route calls it synchronously inside the request, so **the systemd unit must pass `--timeout 300`** — gunicorn's default is 30 s, which killed the worker partway every time (reproducibly at ~269 images for r26). The symptom is misleading: the browser shows "En feil oppstod under generering", which is frontend text from `admin_turnus_sets.html` meaning *the request died*, not that the generator reported anything. Worker timeouts land in `/var/log/turnushjelper/error.log`, **not** journald. Note the route still occupies one of two sync workers for its whole duration, so the site runs at half capacity meanwhile; prefer the CLI for routine work:

  ```bash
  nohup venv/bin/python -c "import app.utils.pdf.strekliste_generator as sg; r=sg.generate_all_images('r26', force=True); print(r['total'], len(r['generated']), r['errors'][:5])" > /tmp/strekliste.log 2>&1 &
  ```

  A `force=True` run builds into a sibling `.png-gen-*` temp directory and swaps it in only once every image is written (`_swap_in`), so **an interrupted run leaves the existing images untouched** rather than destroying them — that holds even for a SIGKILL, which no `try/finally` could catch. Verify with `ls turnusdata/r26/streklister/png | wc -l` — r26 must be **423**; 492 means the wrong PDF edition (want md5 `9091cf5ee5b1fd5db797a6dc3ccf6888`). Per-shift failures do *not* abort the run: they collect in `result["errors"]`, which the admin route discards and the CLI form above prints. Guarded by `tests/test_strekliste_atomic_swap.py`.
- **Never seed a real `year_identifier` in tests that delete a turnus set.** `admin.delete_turnus_set` calls `strekliste_generator.delete_all_images(version)`, which resolves through `AppConfig.turnusfiler_dir` — a test using `"R26"` deletes the committed 423 PNGs on every run, silently (they are gitignored, so `git status` stays clean). `tests/test_turnus_set_delete_guard.py` uses `FAKE_YEAR = "T99"` *and* an autouse fixture repointing `turnusfiler_dir` at `tmp_path`.
