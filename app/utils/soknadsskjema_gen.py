"""Søknadsskjema document generation (docx + PDF).

Extracted from `app/routes/shifts/soknadsskjema.py` — routes hold no
document-generation logic. Both builders take the same arguments and are
called from the `/soknadsskjema` POST handler.

docx and reportlab are imported lazily inside the functions, as they were in
the route, so neither becomes an import-time dependency of the app.
"""
